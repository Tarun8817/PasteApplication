...
import { FormatDate } from "../utlis/fromateDate";
...
const Paste = () => {
  ...
  return (
    <div className="w-full h-full py-10 max-w-[1800px] text-white mx-auto px-5 lg:px-0 bg-black">
      <div className="flex flex-col gap-y-3">
        {/* Search */}
        <div className="w-full  flex gap-3 px-4 py-2  rounded-[0.1rem] border border-[rgba(254, 254, 254, 0.3)]  mt-6">
          <input
            type="search"
        ...
        {/* All Pastes */}
        <div className="flex flex-col border border-[rgba(141, 135, 135, 0.3)] py-4 rounded-[0.4rem]">
          <h2 className="px-4 text-3xl font-bold border-b border-[rgba(144, 132, 132, 0.3)] pb-4">
            All Pastes
          </h2>
          <div className="w-full px-4 pt-4 flex flex-col gap-y-5">
            {filteredPastes.length > 0 ? (
              filteredPastes.map((paste) => (
                <div
                  key={paste?._id}
                  className="border border-[rgba(141, 127, 127, 0.3)] w-full gap-y-6 justify-between flex flex-col sm:flex-row p-4 rounded-[0.3rem]"
                >
